...
check_system(){
    if [[ "${ID}" == "centos" && ${VERSION_ID} -ge 7 ]];then
        ...
        $INS update
        ## 添加 Nginx apt源
    elif [[ "${ID}" == "ubuntu" && `echo "${VERSION_ID}" | cut -d '.' -f1` -ge 16 ]];then#!/bin/bash

#====================================================
#	System Request:Debian 9+/Ubuntu 18.04+/Centos 7+
#	Origin Author:	wulabing
#	Dscription: V2ray ws+tls onekey 
#	Version: E5.1
#	email:wxpiero@gmail.com
#	Official document: www.v2ray.com
#====================================================
...
web_camouflage(){
    ...
    mv startbootstrap-stylish-portfolio 3DCEList
    judge "web 站点伪装"
}
v2ray_install(){
...
domain_check(){
    read -p "请输入你的域名信息(eg:www.IloveXMT.com):" domain
